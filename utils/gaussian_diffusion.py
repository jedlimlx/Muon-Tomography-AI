# blatantly stolen from https://github.com/openai/guided-diffusion/blob/main/guided_diffusion/gaussian_diffusion.py
...
def mean_flat(tensor):
    ...
    """
    return tf.math.reduce_mean(tensor, axis=0)
...
def discretized_gaussian_log_likelihood(x, *, means, log_scales):
    ...
    log_cdf_plus = tf.math.log(cdf_plus.clamp(min=1e-12))
    log_one_minus_cdf_min = tf.math.log((1.0 - cdf_min).clamp(min=1e-12))
    cdf_delta = cdf_plus - cdf_min
    log_probs = tf.where(
        x < -0.999,
        log_cdf_plus,
        tf.where(x > 0.999, log_one_minus_cdf_min, tf.math.log(cdf_delta.clamp(min=1e-12))),
    )
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_output, model_var_values = tf.split(model_output, C, axis=1)
    ...
    def ddim_sample(
        ...
        mean_pred = (
            out["pred_xstart"] * tf.math.sqrt(alpha_bar_prev)
    ...
    def ddim_reverse_sample(
        ...
        mean_pred = (
            out["pred_xstart"] * tf.math.sqrt(alpha_bar_next)
            + tf.math.sqrt(1 - alpha_bar_next) * eps
        )
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                frozen_out = tf.concat([model_output.detach(), model_var_values], axis=1)
            ...
            target = noise[self.model_mean_type]
    ...
    def _prior_bpd(self, x_start):
        ...
        t = tf.constant([self.num_timesteps - 1] * batch_size)
    ...
    def calc_bpd_loop(self, model, x_start, clip_denoised=True, model_kwargs=None):
        ...
        for t in list(range(self.num_timesteps))[::-1]:
            t_batch = tf.constant([t] * batch_size, device=device)
            noise = tf.random.normal(tf.shape(x_start))
        ...
        vb = tf.stack(vb, axis=1)
        xstart_mse = tf.stack(xstart_mse, axis=1)
        mse = tf.stack(mse, axis=1)
