...
def perception_loss(y_pred, y_true, model=model, max_value=1.6):
    shape = tf.shape(y_pred)
    y_pred = tf.reshape(y_pred, (shape[0] * shape[1], shape[2], shape[3], shape[4])) / max_value * 255
    y_true = tf.reshape(y_true, (shape[0] * shape[1], shape[2], shape[3], shape[4])) / max_value * 255
