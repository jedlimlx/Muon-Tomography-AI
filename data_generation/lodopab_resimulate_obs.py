...
# from https://github.com/jleuschn/lodopab_tech_ref/blob/master/resimulate_observations.py
...
def ground_truth_gen(part='train'):
    ...
    for i in range(num_files):
        with h5py.File(
                os.path.join(PATH,
                             'ground_truth_{}'.format(part),
                             'ground_truth_{}_{:03d}.hdf5'.format(part, i)), 'r') as file:
    ...
    for part in ['train', 'validation', 'test']:
        ...
        for filenumber in tqdm(range(n_files), desc=part):
            ...
            with h5py.File(obs_filename, 'w') as observation_file:
                ...
                data_buf = map(forward_fun, im_buf)
